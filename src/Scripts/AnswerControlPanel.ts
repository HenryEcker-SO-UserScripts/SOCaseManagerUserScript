...
const buildBaseTimelineButtons = (mountPoint: JQuery, answerId: number) => {
    ...
    const popOver = $(
        `<div class="s-popover" id="${getTimelinePopoverId(answerId)}" role="menu"><div class="s-popover--arrow"/><div class="${popoverMountPointClass}"><div class="is-loading">Loading…</div></div></div>`
    );
    mountPoint.append(controlButton);
    mountPoint.append(popOver);
};
                ...
                .then(timelineEvents => {
                    ...
                    for (const event of timelineEvents) {
                        timelineEventContainer.append(
                            $(`<div class="flex--item d-flex fd-row jc-space-between ai-center"><a href="/users/${event['account_id']}">${event['display_name']}</a><span data-event-type-id="${event['timeline_event_type']}">${event['timeline_event_description']}</span><span>${new Date(event['event_creation_date']).toLocaleString()}</span></div>`)
                        );
                    }
