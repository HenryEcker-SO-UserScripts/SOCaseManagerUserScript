...
(function() {
    ...
    function registerAbsoluteLinkReducer() {
        ...
        const shortestRelativeLinkReducer = [ s => s.replace(absoluteLinkPattern, "[$1](/$2)"), s => s.replace(/\[(.*?)]\(\/questions\/(\d+)\/[^/#]+(?:\?.+?)?\)/g, "[$1](/q/$2)"), s => s.replace(/\[(.*?)]\(\/questions\/\d+\/.+?#(\d+)(?:\?.+?)?\)/g, "[$1](/a/$2)"), s => s.replace(/\[(.*?)]\(\/questions\/\d+\/.+?#comment(\d+)_\d+\)/g, "[$1](/posts/comments/$2)"), s => s.replace(/\[(.*?)]\(\/users\/(\d+)\/[^?]+(\?tab=.+?)?\)/g, ((sub, p1, p2, p3) => {
            if (void 0 === p3 || "?tab=profile" === p3) {
                return `[${p1}](/users/${p2})`;
            }
        ...
        Stacks.addController("socm-absolute-link-reducer", {
            target: [ "text-area" ],
            handleReduceText(ev) {
